$(document).ready(function(){
    ...
    var jokes = [
      "You're so slow, the rabbit grew a beard while racing... go look..., GOTCHA!!! didn't I?",
    ...
    $(window).keydown(function (e) {
      ...
      if(count > 3000){
        ...
        if (e.keyCode === 37) {
          if (position.left > 0) {
            $('.rabbit').css('left', Math.max(position.left - 50, 0));
          }
        }
        if (e.keyCode === 38) {
          if (position.top > 0) {
            $('.rabbit').css('top', Math.max(position.top - 50, 0));
          }
        }
        if (e.keyCode === 39) {
          if (position.left + $('.rabbit').width() < window.innerWidth) {
            $('.rabbit').css('left', Math.min(position.left + 50, window.innerWidth - $('.rabbit').width()));
          }
        }
        if (e.keyCode === 40) {
          if (position.top + $('.rabbit').height() < window.innerHeight) {
            $('.rabbit').css('top', Math.min(position.top + 50, window.innerHeight - $('.rabbit').height()));
          }
        }
      }
    });
